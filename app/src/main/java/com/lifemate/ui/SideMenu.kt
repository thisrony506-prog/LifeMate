...
@Composable private fun DrawerEntry(label: String, icon: ImageVector, destination: String, route: String, navigate: (String)->Unit) {
    NavigationDrawerItem(label={Text(label)},icon={Icon(icon,null)},selected=route==destination,onClick={navigate(destination)},modifier=Modifier.testTag("drawer-$destination"))
}
