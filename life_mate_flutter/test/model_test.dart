...
void main() {
  test('all module models round-trip without losing fields', () {
    for (final kind in EntryKind.values) {
      final e = Entry(
        ...
        fields: {'amount': 120.5, 'body': 'A private note', 'done': true},
      );
      final restored = Entry.fromJson(e.toJson());
      expect(restored.id, e.id);
      expect(restored.kind, kind);
      expect(restored.fields, e.fields);
    }
  });
  test('future schema fails closed', () {
    final json = Entry(kind: EntryKind.journal, title: 'secret').toJson()
      ..['schema'] = 2;
    ...
    final list = [
      for (var i = 0; i < 3; i++)
        Entry(
          ...
          fields: {
            'done': true,
            'completedDay': dayKey(now.subtract(Duration(days: i))),
          },
        ),
    ];
  ...
  test('demo data never enters persistence or outbox', () async {
    ...
    await expectLater(
      s.save(Entry(kind: EntryKind.task, title: 'x')),
      throwsStateError,
    );
    s.setDemo(false);
    expect(s.all, isEmpty);
  ...
  test(
    'deletions retain a sync tombstone; health and contacts never auto-sync',
    () {
      final e = Entry(
  ...
  test('offline save and completion update statistics', () async {
    final s = LifeStore.memory();
    final e = Entry(kind: EntryKind.task, title: 'One thing');
    await s.save(e);
    await s.toggle(e);
    expect(s.dailyStreak, 1);
    await s.save(
      Entry(kind: EntryKind.expense, title: 'Fare', fields: {'amount': 120}),
    );
