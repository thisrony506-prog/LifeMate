...
void main() {
  test(
    'indexed reads remain correct and caller sorting cannot corrupt cached order',
    () async {
      ...
      print(
        'PERF indexed 100 reads / 1000 records: ${watch.elapsedMicroseconds} us (test host)',
      );
      store.entries(EntryKind.expense).clear();
      expect(store.entries(EntryKind.expense).map((e) => e.id), expected);
      await store.save(
        Entry(kind: EntryKind.income, title: 'Income', fields: {'amount': 500}),
      );
