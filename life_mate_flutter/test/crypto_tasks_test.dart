...
void main() {
  test(
    'isolated derivation preserves existing key format and lets UI loop run',
    () async {
      ...
      late SecretKey key;
      try {
        key = await derivePhraseKey(phrase, salt);
      } finally {
        timer.cancel();
      }
      expect(ticks, greaterThan(0));
      final previous = await Pbkdf2(
  ...
  test(
    'isolated media is compatible, authenticated and rejects a wrong key',
    () async {
      ...
      await expectLater(
        decryptMedia(encrypted, List.filled(32, 255)),
        throwsA(isA<SecretBoxAuthenticationError>()),
      );
      encrypted[15] ^= 1;
      await expectLater(
        decryptMedia(encrypted, key),
        throwsA(isA<SecretBoxAuthenticationError>()),
      );
    },
  );
