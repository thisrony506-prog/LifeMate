...
void main() {
  test(
    'portable backup authenticates and adds records without deleting existing work',
    () async {
      final dir = await Directory.systemTemp.createTemp('life-backup');
      Hive.init(dir.path);
      ...
      await expectLater(
        VaultBackup.restore(store, bytes, 'wrong recovery phrase'),
        throwsA(isA<SecretBoxAuthenticationError>()),
      );
      expect(store.all.length, 1);
      await VaultBackup.restore(store, bytes, 'correct recovery phrase');
      expect(store.all.length, 2);
      final corrupt = Uint8List.fromList(bytes);
      corrupt[corrupt.length ~/ 2] ^= 1;
      await expectLater(
        VaultBackup.restore(store, corrupt, 'correct recovery phrase'),
        throwsA(anything),
      );
      expect(store.all.length, 2);
      await box.close();
