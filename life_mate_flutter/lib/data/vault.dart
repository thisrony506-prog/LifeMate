...
class Vault {
  ...
  static Future<Vault> open() async {
    ...
    if (exists && encoded == null)
      throw StateError(
        'The device encryption key is unavailable. Existing data was not replaced.',
      );
  ...
  List<Entry> get entries => box.keys
      .where((k) => k.toString().startsWith('entry:'))
      .map(
        (k) => Entry.fromJson(jsonDecode(box.get(k)!) as Map<String, dynamic>),
      )
      .toList();
  Future<void> put(Entry entry) =>
      box.put('entry:${entry.id}', jsonEncode(entry.toJson()));
  Future<void> setting(String key, String value) =>
      box.put('setting:$key', value);
  String settingValue(String key, [String fallback = '']) =>
      box.get('setting:$key', defaultValue: fallback)!;
