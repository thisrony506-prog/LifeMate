...
class AppUpdates extends ChangeNotifier {
  bool ready = !NativeBridge.android, required = false, checking = false, busy = false, downloaded = false;
  ...
  Future<void> load() async {
    ...
    try { accept(await NativeBridge.channel.invokeMapMethod('updates.state') ?? {}); }
    catch (_) { status = 'startup'; notifyListeners(); } // Fail closed if native gate state cannot be read.
  }
  Future<void> check({bool manual = false}) async {
    if (!NativeBridge.android || checking) return;
    checking = true; notifyListeners();
    try {
      accept(await NativeBridge.channel.invokeMapMethod('updates.check', {'manual': manual}) ?? {});
  ...
  Future<void> download() async {
    ...
    busy = true; downloaded = false; status = ''; notifyListeners();
    try { await NativeBridge.channel.invokeMethod('updates.download'); downloaded = true; status = 'ready'; }
    catch (_) { status = 'downloadError'; }
    finally { busy = false; notifyListeners(); }
  }
  Future<void> install() async {
    if (busy || !required) return;
    busy = true; notifyListeners();
    try { status = await NativeBridge.channel.invokeMethod<String>('updates.install') ?? 'installer'; }
    catch (_) { downloaded = false; status = 'installError'; }
    finally { busy = false; notifyListeners(); }
  }
}
