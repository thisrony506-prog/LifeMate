...
class CloudService {
    ...
    if (Firebase.apps.isEmpty)
      await Firebase.initializeApp(
        options: FirebaseOptions(
          ...
          iosBundleId: 'com.lifemate',
        ),
      );
  }

  static Future<User> account(
    String email,
    String password,
    bool create,
  ) async {
  ...
  static Future<String> unseal(String text, SecretKey key) async => utf8.decode(
  ...
  static Future<void> sync(LifeStore store, String phrase) async {
    ...
    if (check.exists) {
      if (await unseal(check.data()!['ciphertext'] as String, secret) !=
          'LifeMate-v1')
        throw StateError('Incorrect recovery phrase');
    } else {
      final sealed = await seal('LifeMate-v1', secret);
      await db.runTransaction((tx) async {
        if ((await tx.get(marker)).exists)
          throw StateError('Vault changed. Retry sync.');
        tx.set(marker, {'ciphertext': sealed});
      });
    }
    await store.vault?.setting('syncOwner', user.uid);
    ...
    for (final doc in remote.docs) {
      final entry = Entry.fromJson(
        jsonDecode(await unseal(doc.data()['ciphertext'] as String, secret))
            as Map<String, dynamic>,
      );
      final revision = doc.data()['revision'] as int;
      if (entry.id != doc.id || entry.revision != revision || !entry.syncable)
        throw const FormatException('Invalid encrypted record binding');
      final existing = local[entry.id];
      if (existing != null &&
          (revision < existing.revision ||
              existing.dirty && existing.revision != revision)) {
        throw StateError(
          store.t(
            'Sync conflict. Both copies are safe; export before resolving on the other device.',
            'সিঙ্কে দ্বন্দ্ব। দুই কপিই নিরাপদ আছে; অন্য ডিভাইসে সমাধানের আগে ব্যাকআপ রাখো।',
          ),
        );
      }
      if (existing == null || !existing.dirty)
        await store.acceptCloud(entry.copy(revision: revision, dirty: false));
    }
    for (final entry in List<Entry>.from(store.pending)) {
      ...
      await db.runTransaction((tx) async {
        ...
        if (revision != entry.revision)
          throw StateError('Sync conflict: local edits were retained');
        tx.set(ref, {
          'ciphertext': cipher,
      ...
      if (identical(current, entry)) {
        await store.acceptCloud(next);
      } else if (current != null) {
        await store.acceptCloud(
          current.copy(revision: next.revision, dirty: true),
        );
      }
    }
  }
  ...
  static Future<void> uploadEncryptedPhoto(
    String id,
    List<int> bytes,
    SecretKey key,
  ) async {
    ...
    await FirebaseStorage.instance
        .ref('users/${user.uid}/encrypted/$id')
        .putData(
          encrypted,
