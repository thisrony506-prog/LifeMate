...
class FamilyShare {
  ...
  static Future<Position> position() async {
    if (!await Geolocator.isLocationServiceEnabled())
      throw StateError('Location is off');
    var permission = await Geolocator.checkPermission();
    if (permission == LocationPermission.denied)
      permission = await Geolocator.requestPermission();
    if (permission == LocationPermission.denied ||
        permission == LocationPermission.deniedForever)
      throw StateError('Location permission is off');
  ...
  Future<void> _send(String uid) async {
    ...
    if (id != share) return;
    final cipher = await CloudService.seal(
      jsonEncode({
    ...
    await FirebaseFirestore.instance.doc('live/$share').set({
      'owner': uid,
      'ciphertext': cipher,
  ...
  Future<void> stop() async {
    ...
    if (old != null) {
      try {
        ...
        error =
            'Could not revoke remotely. The invitation still expires after 15 minutes.';
      }
    }
  }
  ...
  static Stream<Map<String, dynamic>?> watch(String invitation) async* {
    ...
    if (parts.length != 2 || !RegExp(r'^[a-f0-9-]{36}$').hasMatch(parts[0]))
    ...
    await for (final snapshot
        in FirebaseFirestore.instance.doc('live/${parts[0]}').snapshots()) {
      ...
      yield {
        ...point,
        'expires': (data['expiresAt'] as Timestamp).toDate().toIso8601String(),
      };
    }
  }
}
