...
class DeviceLock extends StatefulWidget {
  ...
  const DeviceLock({super.key, required this.child, this.enabled = true, this.allowBack = true});
...
class _DeviceLockState extends State<DeviceLock> with WidgetsBindingObserver {
  ...
  @override void initState() {
    ...
    if (widget.enabled) WidgetsBinding.instance.addPostFrameCallback((_) => unlock());
  }
  @override void dispose() { WidgetsBinding.instance.removeObserver(this); super.dispose(); }
  @override void didChangeAppLifecycleState(AppLifecycleState state) {
    if (widget.enabled && state != AppLifecycleState.resumed && !authenticating && mounted) setState(() => locked = true);
  }
  Future<void> unlock() async {
    ...
    try {
      ...
      final ok = await LocalAuthentication().authenticate(localizedReason: s.t('Unlock your private Life Mate space', 'Life Mate-এর ব্যক্তিগত জায়গা খোলো'), options: const AuthenticationOptions(stickyAuth: true));
      if (mounted) setState(() => locked = !ok);
    } catch (_) { if (mounted) setState(() => locked = true); }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Stack(children: [
      ExcludeFocus(excluding: hidden, child: Offstage(offstage: hidden, child: widget.child)),
      if (hidden) Positioned.fill(child: Scaffold(body: SafeArea(child: Center(child: Padding(padding: const EdgeInsets.all(28), child: Column(mainAxisSize: MainAxisSize.min, children: [
        const Icon(Icons.lock_outline, size: 48), const SizedBox(height: 20),
        Text(s.t('Your space, kept safe.', 'তোমার জায়গা, সুরক্ষিত।'), style: Theme.of(context).textTheme.headlineSmall),
