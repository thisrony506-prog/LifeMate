...
**Your Personal Life Assistant.** A native, offline-first Android app built with Kotlin, Jetpack Compose, and Material 3. Premium white/black/pink light styling, a neutral charcoal dark theme, and real local data—no static screens or seeded production demo records.

## Installable APKs and safe updates

**[Download signed LifeMate 1.2.64 (Android 8+)](https://github.com/thisrony506-prog/LifeMate/releases/download/v1.2.64/LifeMate-100064.apk)** — versionCode `100064`, approximately 22.9 MB. Old unsigned 1.1.x downloads cannot be installed; use this signed release instead.
...
**Verified:** [run 34711233177](https://github.com/thisrony506-prog/LifeMate/actions/runs/34711233177), application source `642089b`, passed all three jobs: compilation/lint, **54 JVM test methods**, **17 script tests**, Android feature/UI/card/post/migration tests and actual process-exit/reboot/recurrence checks. The signed emulator test upgraded the published **1.2.55 → 1.2.64**, preserving its encrypted profile through Room's additive migration. Publication completed automatically; this run contains exactly one **LifeMate-Release-APK** artifact. See [verification evidence and physical-device limits](docs/VERIFICATION.md). Older installed builds (1.2.52 and earlier) need this APK installed once before gaining the browser-free updater.

## A cleaner, more personal LifeMate

- A unified white/black/pink interface, compact Home, five primary tabs and a direct hamburger menu. Lazy Memories grid and routine time sorting retain existing filtering/CRUD.
- **Facebook Posts:** ten offline caption types, editable persistent drafts, optional photos/name/date, copy/share and reusable graphics. Optional AI is explicitly consented and needs a trusted server-side backend; no provider key or account is required for offline use.
- Optional short completion/success sounds are off by default; existing notification sound/vibration controls remain.
...
See the [premium interface, posts, migration and optional AI contract](docs/PREMIUM.md) and [updater/studio device checks](docs/UX-UPDATE.md). Studio drafts/photos are not in record ZIP backups: save completed images separately. Existing organizer features and the supplied logo are retained.

## Design and branding

The user-supplied blue person, green leaves and yellow rays logo now appears on the adaptive launcher icon, Android 13 themed icon, welcome/loading screens, dashboard, inner-screen header and About card. Matching single-color icons identify notifications. The original upload is retained as `20260912_111618.png`; `bash scripts/update-brand-assets.sh` reproducibly sizes it for Android without redrawing it. Clear sans-serif headings, coordinated light/dark colors, consistent outlined icons, compact feature cards and an animated live progress ring make the existing offline tools easier to explore. Profile editing remains accessible through the dashboard shortcut and Profile tab. No sample records are inserted into the shipping app.
...
- **Wishes & cards:** seven offline tones, editable personalized messages, copy/share, flat White/Black/Pink and retained creative palettes, five font styles, real rendered PNG preview, save/share image. Messages are never automatically sent.
...
- **Memories:** adaptive lazy photo grid, filters/sorting, system photo/video picker, private local copies, captions/dates/notes, image preview, in-app system VideoView playback, explicit sharing.
...
- **Calendar:** date navigation, pink event indicators including memories, details navigation, selected-day event creation and check-ins.
...
- **Settings:** optional post AI endpoint, local sound effects, notification access, exact-alarm access, global sound/vibration, appearance, profile, app lock/strong biometrics, export/import/backup/restore, typed-confirmation delete-all, privacy, terms, support.
...
domain/         Recurrence/streak rules, offline post templates, optional AI boundary
...
- No analytics, ad SDK, account requirement, automatic media upload or automatic cloud backup. INTERNET serves public update metadata, requested official APK downloads and optional explicitly consented post generation at a user-configured HTTPS backend. Updates send no personal records. AI sends only the approved post writing fields (which may contain personal information), never photos or unrelated records. Foreground update checks are rate-limited; verified newer versions require installation.
...
- ZIP backups are **unencrypted**, clearly warned before export, include profile/records/completions/media and saved posts/photos, and exclude PIN/device settings. Restore validates version, paths, archive expansion limits, dates, IDs, and relationships before an atomic database replacement. Invalid restore leaves current data intact.
...
`PostGenerator` and the bounded HTTPS `BackendPostGenerator` support explicitly consented text-only post generation. Configure a trusted backend in Settings; keep all provider keys server-side. Offline templates remain available. No live backend was supplied/deployed or tested end-to-end. See [request/response and privacy details](docs/PREMIUM.md). The earlier `AssistantProvider` boundary remains available for future organizer integrations.
